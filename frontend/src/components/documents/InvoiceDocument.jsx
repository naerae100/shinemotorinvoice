...
export default function InvoiceDocument({ invoice, settings }) {
  ...
  return (
    <div className="print-sheet relative mx-auto flex min-h-[297mm] flex-col bg-white p-6 sm:p-10 lg:p-10 text-[13px] sm:shadow-lg sm:ring-1 sm:ring-ink-200">
      ...
      <section className="mt-5 grid grid-cols-2 rounded-lg bg-brand-50/50 border border-brand-100 p-4 sm:grid-cols-4">
      ...
      <div className="mt-6 overflow-x-auto print:overflow-visible rounded-t-lg ring-1 ring-ink-200">
        <table className="w-full min-w-[520px] text-[13px] border-separate border-spacing-0">
          <thead>
            <tr className="bg-brand-900 text-left uppercase tracking-[0.15em] text-white">
              <th className="w-10 rounded-tl-lg px-4 py-2.5 print:py-2 text-[10px] font-bold">#</th>
              <th className="px-2 py-2.5 print:py-2 text-[10px] font-bold">Description of goods</th>
              <th className="w-24 px-2 py-2.5 print:py-2 text-right text-[10px] font-bold">Weight (MT)</th>
              <th className="w-28 px-2 py-2.5 print:py-2 text-right text-[10px] font-bold">Unit price ({currency})</th>
              <th className="w-32 rounded-tr-lg px-4 py-2.5 print:py-2 text-right text-[10px] font-bold">Amount ({currency})</th>
            </tr>
          </thead>
          <tbody className="bg-white">
            {invoice.lineItems.map((li, i) => {
              ...
              return (
                <tr key={li.id} className={i % 2 ? 'bg-ink-50/50' : ''}>
                  <td className="num border-b border-ink-100 px-4 py-2.5 print:py-2 align-top text-ink-500 font-medium">
                    {i + 1}
                  </td>
                  <td className="border-b border-ink-100 px-2 py-2.5 print:py-2 align-top text-[13px] font-bold text-ink-900">
                    {heading}
                    {li.packageCount && (
                  ...
                  <td className="num border-b border-ink-100 px-2 py-2.5 print:py-2 text-right align-top font-semibold text-ink-800">
                    {formatNumber(li.netWeightMt, 3)}
                  </td>
                  <td className="num border-b border-ink-100 px-2 py-2.5 print:py-2 text-right align-top font-semibold text-ink-800">
                    {formatNumber(li.pricePerMt, 2)}
                  </td>
                  <td className="num border-b border-ink-100 px-4 py-2.5 print:py-2 text-right align-top font-bold text-[13px] text-ink-900">
          ...
          <tfoot className="bg-brand-50/30">
            <tr>
              <td />
              <td className="px-2 py-3 print:py-2 text-[11.5px] font-bold uppercase tracking-[0.15em] text-brand-900">
                Total {invoice.lineItems.length} {invoice.lineItems.length === 1 ? 'item' : 'items'}
              </td>
              <td className="num px-2 py-3 print:py-2 text-right font-bold text-brand-900">
                {formatNumber(totalWeight, 3)}
              </td>
              <td />
              <td className="num px-4 py-3 print:py-2 text-right font-bold text-[13.5px] text-brand-900">
      ...
      <section className="avoid-break mt-6 flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row justify-between items-end gap-6">
          <div className="flex-1">
            <div className="mb-2 text-[10px] font-bold uppercase tracking-[0.15em] text-brand-700">
        ...
        <div className="rounded-lg bg-brand-50/50 p-4 border border-brand-100">
          <div className="mb-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-brand-700 text-center">
            Declaration
          </div>
