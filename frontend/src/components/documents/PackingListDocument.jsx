...
export default function PackingListDocument({ invoice, settings }) {
  ...
  return (
    <div className="print-sheet relative mx-auto flex min-h-[297mm] flex-col rounded-2xl border border-ink-200 bg-white p-5 shadow-ticket sm:p-8 lg:p-10">
      ...
      {!weighed && (
        <div className="print-hidden mt-3 rounded-xl border border-working-amber/30 bg-working-amberDim px-4 py-2.5 text-[11px] font-medium text-ink-800">
            ...
            <Fragment key={group.container?.id ?? `loose-${gi}`}>
              {containers.length > 1 && (
                <tr className="bg-ink-100">
                  <td colSpan={5} className="border-b border-ink-200 px-3 py-2 print:py-1">
                    <span className="num text-[10.5px] font-bold text-ink-900">
              ...
              {group.lines.map((li) => {
                ...
                return (
                  <tr key={li.id} className={serial % 2 === 0 ? 'bg-ink-50' : ''}>
      ...
      <section className="avoid-break grid grid-cols-1 gap-8 pt-5 sm:grid-cols-2">
        <div>
          ...
          <p className="text-[11px] font-medium leading-relaxed text-ink-700">
            All the goods are from Australia.
          </p>
          <p className="mt-2 text-[10.5px] leading-relaxed text-ink-600">
